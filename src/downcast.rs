...
fn safe_shrink_integers(
    ...
    //     let non_int_shrink_expr =  all().exclude_cols(&int_columns).shrink_type();
...
fn convert_floats_to_integers(
    ...
    let check_exprs: Vec<Expr> = float_columns
        ...
        .map(|col_name| {
            ...
            // Check if round-trip casting preserves all values
            original_col.eq(back_to_float).all(false).alias(&format!("{}_can_convert", col_name))
        })
        .collect();
