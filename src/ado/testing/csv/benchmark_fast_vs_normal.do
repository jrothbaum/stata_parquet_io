...
tempfile base
tempfile csv_tmp
local csv_file "`csv_tmp'.csv"
capture erase "`csv_file'"

di as text "Benchmark: pq use_csv  fast (cached) vs  normal (streaming)"
...
foreach n of local sizes {
	...
	forvalues r = 1/`reps' {
		clear
		timer on 2
		pq use_csv using "`csv_file'", clear fast
		timer off 2
		assert _N == `n'
	}
