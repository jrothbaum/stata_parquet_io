...
capture pq save_csv "`tcsv'.csv", replace chunk(1000)
assert _rc != 0

// Test 3: parquet-only chunk+consolidate should fail on CSV
capture pq save_csv "`tcsv'.csv", replace chunk(1000) consolidate
assert _rc != 0

// Test 4: parquet-only stream should fail on CSV
capture pq save_csv "`tcsv'.csv", replace chunk(1000) stream
assert _rc != 0

// Test 5: parquet-only stream+do_not_reload should fail on CSV
capture pq save_csv "`tcsv'.csv", replace chunk(1000) stream do_not_reload
assert _rc != 0
