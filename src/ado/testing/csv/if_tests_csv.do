...
program define in_test_parquet_io_data
	...
	pq save_csv "`path_save_root'.csv", replace
	...
	// Keep CSV if-test runtime short: run a focused subset of cases.
	local n_if_run = min(`n_if', 5)
	forvalues i = 1/`n_if_run' {
		di "`i'"
		pq use_csv "`path_save_root'.csv", clear 
		count
		di `"keep if `if_set`i''"'
		keep if `if_set`i''
		sort c_1
		...
		di "Test if on load"
		pq use_csv "`path_save_root'.csv", clear if(`if_set`i'')
		...
		di "Test if on save"
		pq use_csv "`path_save_root'.csv", clear
		pq save_csv "`path_save_root'_subset.csv", replace if(`if_set`i'')
		
		pq use_csv "`path_save_root'_subset.csv", clear
		capture unab all_vars: *
		if (_rc) {
			assert _N == 0
	...
	pq use_csv "`path_save_root'.csv", clear
	pq save_csv c_1 c_2 c_3 using "`path_save_root'_subset.csv", replace
	pq use_csv "`path_save_root'_subset.csv", clear
	sum
	
	
	pq use_csv "`path_save_root'.csv", clear
	pq save_csv c_* using "`path_save_root'_subset.csv", replace
	pq use_csv "`path_save_root'_subset.csv", clear
	sum
	
	pq use_csv "`path_save_root'.csv", clear
	pq save_csv c_1-c_3 using "`path_save_root'_subset.csv", replace
	pq use_csv "`path_save_root'_subset.csv", clear
	sum
	...
	capture erase `path_save_root'.csv
	capture erase `path_save_root'_subset.csv
