...
program define in_test_parquet_io_data
	...
	forvalues i = 1/`n_in' {
		...
		pq use_csv "`path_save_root'.csv", clear in(`in_set`i'')
		...
		di "Test in on save"
		pq use_csv "`path_save_root'.csv", clear
		pq save_csv "`path_save_root'_subset.csv", replace in(`in_set`i'')
	...
	pq use_csv "`path_save_root'.csv", clear in(`out_of_range_start'/`out_of_range_end')
	count
	sum
	di "Rows: " _N
