...
describe
tempfile t_save
pq save_spss "`t_save'.sav", replace
...
gen gamma_ = _N - _n
order beta_
pq save_spss "`t_save'_append.sav", replace

pq use_spss "`t_save'.sav", clear
gen alpha_ = _n
gen omega_ = _N - _n
order alpha_
count
pq append "`t_save'_append.sav", format(spss)
count
...
capture erase "`t_save'.sav"
capture erase "`t_save'_append.sav"
