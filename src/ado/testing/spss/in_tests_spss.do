...
program define in_test_parquet_io_data
	...
	forvalues i = 1/`n_in' {
		...
		pq use_spss "`path_save_root'.sav", clear in(`in_set`i'')
		...
		di "Test in on save"
		pq use_spss "`path_save_root'.sav", clear
		pq save_spss "`path_save_root'_subset.sav", replace in(`in_set`i'')
	...
	pq use_spss "`path_save_root'.sav", clear in(`out_of_range_start'/`out_of_range_end')
	count
	sum
	di "Rows: " _N
