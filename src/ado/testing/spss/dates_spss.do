...
//	pq save_spss → pq use_spss with exact equality.
...
pq save_spss "`pq1'.sav", replace


//	----------------------------------------------------------------------
//	Test 1: full load — values must be bit-for-bit identical
//	----------------------------------------------------------------------
pq use_spss "`pq1'.sav", clear
...
//	----------------------------------------------------------------------
pq use_spss "`pq1'.sav", clear if(date_var >= date('05jan2020','%d%b%Y'))
...
pq use_spss "`pq1'.sav", clear
tempfile pq2
pq save_spss "`pq2'.sav", replace if(date_var <= date('03jan2020','%d%b%Y'))

pq use_spss "`pq2'.sav", clear
assert _N == 3
assert date_var[1] == td(01jan2020)
...
//	----------------------------------------------------------------------
capture pq use_spss "`pq1'.sav", clear if(date_var >= td(05jan2020))
assert _rc == 198
...
//	----------------------------------------------------------------------
pq use_spss "`pq1'.sav", clear if(datetime_var >= TIMESTAMP '2020-01-03 00:00:00')
...
//	----------------------------------------------------------------------
capture pq use_spss "`pq1'.sav", clear if(datetime_var >= tc(03jan2020 00:00:00))
assert _rc == 198
