...
program define in_test_parquet_io_data
	...
	pq save_spss "`path_save_root'.sav", replace
	...
	//	(Currently unreached: n_if_run caps at 5, kept in sync with if_tests.do.)
	local n_if = `n_if' + 1
	...
	// Keep SPSS if-test runtime short: run a focused subset of cases.
	local n_if_run = min(`n_if', 5)
	forvalues i = 1/`n_if_run' {
		di "`i'"
		pq use_spss "`path_save_root'.sav", clear 
		count
		di `"keep if `if_set`i''"'
		keep if `if_set`i''
		sort c_1
		...
		di "Test if on load"
		pq use_spss "`path_save_root'.sav", clear if(`if_set`i'')
		...
		di "Test if on save"
		pq use_spss "`path_save_root'.sav", clear
		pq save_spss "`path_save_root'_subset.sav", replace if(`if_set`i'')
		
		pq use_spss "`path_save_root'_subset.sav", clear
		capture unab all_vars: *
		if (_rc) {
			assert _N == 0
	...
	pq use_spss "`path_save_root'.sav", clear
	pq save_spss c_1 c_2 c_3 using "`path_save_root'_subset.sav", replace
	pq use_spss "`path_save_root'_subset.sav", clear
	sum
	
	
	pq use_spss "`path_save_root'.sav", clear
	pq save_spss c_* using "`path_save_root'_subset.sav", replace
	pq use_spss "`path_save_root'_subset.sav", clear
	sum
	
	pq use_spss "`path_save_root'.sav", clear
	pq save_spss c_1-c_3 using "`path_save_root'_subset.sav", replace
	pq use_spss "`path_save_root'_subset.sav", clear
	sum
	...
	capture erase `path_save_root'.sav
	capture erase `path_save_root'_subset.sav
...
program compare_files
	...
	foreach vari in `varlist' {
		...
		if ("`do_assert'" != "" & "`vari'" != "test_datetime") {
