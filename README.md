...
| `statametadata` | Embed Stata variable/value labels, display formats, notes, the dataset label, and storage types while keeping numeric columns numeric |

`statametadata` supports ordinary, partitioned, chunked, streamed, and consolidated Parquet output, and every physical fragment carries one semantically identical metadata envelope. Since display formats and storage types are metadata too and every variable has both, the capsule is written whenever the option is given, including for data with no labels or notes; omit the option for ordinary metadata-free Parquet. `pq use` restores variable labels, value labels, display formats, variable notes, the dataset label, dataset notes, and the original storage types; `pq append` restores metadata for newly created variables only and leaves the existing dataset label and dataset notes alone. Storage types are restored by a post-load recast that Stata performs only when every value fits exactly, so a file rewritten by another tool keeps the wider loaded type rather than losing values. `compress` and `compress_string_to_numeric` leave all storage types as read; `cast()` exempts only the columns it names. Directory and glob reads validate every footer before loading data, then apply the agreed capsule once. Mixed, missing, malformed, or conflicting fragment metadata returns an error without clearing the current dataset. `nostatametadata` is the explicit data-only escape hatch. `label` retains its older decode-to-string behavior and cannot be combined with `statametadata`; the combination of `partition_by()` with `consolidate` cannot be combined with it.
